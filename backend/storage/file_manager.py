...
class FileManager:
    ...
    def get_save_path(
        ...
        *,
        ...
        ``category_path`` is an optional slash-separated path (e.g.
        ``我的收藏/视频``) used by the Electron GUI when downloading items from
        the favorites page. When provided it replaces the author/mode layers
        entirely so collection content is grouped by category instead of by
        author.
        """
        ...
        if isinstance(category_path, str):
            ...
            if sanitized_parts:
                sanitized_category = "/".join(sanitized_parts)

        if sanitized_category:
            save_dir = self.base_path / sanitized_category
        else:
