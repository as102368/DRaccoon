...
class TranscriptManager:
    ...
    def build_output_paths(self, video_path: Path) -> Tuple[Path, Path, Path, Path]:
        video_path = Path(video_path)
        ...
        return (
            ...
            output_dir / f"{stem}.transcript.srt",
            output_dir / f"{stem}.transcript.vtt",
        )
    ...
    async def process_video(self, video_path: Path, aweme_id: str) -> Dict[str, Any]:
        ...
        text_path, json_path, srt_path, vtt_path = self.build_output_paths(video_path)
        model = self._model()
        ...
        try:
            ...
            try:
                ...
                await self._write_outputs(payload, text_path, json_path, srt_path, vtt_path)
                await self._record_job(
                    aweme_id=aweme_id,
                    video_path=video_path,
                    transcript_dir=text_path.parent,
    ...
    def _segments_to_vtt(self, segments: List[Dict[str, Any]]) -> str:
        ...
        for seg in segments:
            ...
            if not text:
                continue
            lines.append(
                f"{self._seconds_to_srt_time(start).replace(',', '.')} --> {self._seconds_to_srt_time(end).replace(',', '.')}"
            )
            lines.append(text)
            lines.append("")
        return "\n".join(lines).strip()
    ...
    async def _call_openai_transcription(
        ...
        *,
        ...
        # Use verbose_json with segment timestamps so we can generate txt/json/srt/vtt
