...
class _ServerDeps:
    ...
    def __init__(self, config: ConfigLoader, profile: Optional[str] = None):
        self.config = config
        ...
        self.cookie_manager = CookieManager(cookie_file=cookie_file, profile=profile)
...
async def _execute_download(url: str, deps: "_ServerDeps") -> Dict[str, int]:
    ...
    _ServerDeps 共享。
    """
    proxy_pool = ProxyPool.from_config(deps.config)
    async with DouyinAPIClient(
        deps.cookie_manager.get_cookies(),
        proxy=ProxyPool.single_proxy_from_config(deps.config),
...
def build_app(config: ConfigLoader, profile: Optional[str] = None) -> FastAPI:
    deps = _ServerDeps(config, profile=profile)
...
async def run_server(
    config: ConfigLoader, *, host: str, port: int, profile: Optional[str] = None
) -> None:
    import uvicorn

    app = build_app(config, profile=profile)
