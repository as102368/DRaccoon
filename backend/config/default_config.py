...
DEFAULT_CONFIG: Dict[str, Any] = {
    ...
    "end_time": "",
    "folderstyle": False,
    # 命名模板：渲染时可用变量见 utils/naming.py:ALLOWED_VARIABLES。
    ...
    #   True  - 作者目录下再分 post/like/...
    #   False - 不分模式层，文件直接落在作者目录下（无 POST 文件夹）
    "group_by_mode": False,
    ...
    # 评论采集（可选）。启用后每个作品会额外生成 *_comments.json / *_comments.csv。
    "comments": {
        "enabled": False,
    ...
    "live": {
        ...
        "poll_interval_seconds": 30,  # 自动录制时房间状态轮询间隔
        "segment_duration_seconds": 3600,  # 单个分片时长，0=不分片
        "auto_start": True,  # live_monitor 启动后是否自动开始监控
        "output_format": "mp4",  # 输出格式：mp4 或 flv
        "ffmpeg_path": "",  # 空则使用 imageio_ffmpeg 自动定位
        "max_monitor_rooms": 10,  # 最大并发监控房间数
        "reconnect_delay_seconds": 5,  # 分片间重连间隔
    },
