  ...
  const startWatchdogs = () => {
    ...
    if (maxRuntimeMs > 0) {
      maxRuntimeTimer = setTimeout(() => {
        if (state.finished) return;
        sendLog({ taskId, line: `任务超过最大运行时间 ${maxRuntimeMs}ms，强制终止` });
        killProcessTree(proc.pid);
...
const QUEUE_URL_MAX_RUNTIME_MS = 3 * 60 * 1000; // 单 URL 最多 3 分钟
  ...
  _runOne(subId, jobFile, sendProgressAsRoot, sendLogAsRoot) {
    return new Promise((resolve) => {
      runPythonBridge(
        ...
        {
          maxRuntimeMs: QUEUE_URL_MAX_RUNTIME_MS,
...
ipcMain.handle('sync:start', async (_event, payload) => {
  ...
  const pythonArgs = [
    syncPath,
    '--sync-job', jobFile,
    '--stdout-log', syncOutPath,
    '--stderr-log', syncErrPath,
  ];

  let proc;
  try {
    proc = spawn(pythonPath, pythonArgs, {
      cwd: backendRoot,
      env: { ...process.env, PYTHONIOENCODING: 'utf-8' },
      windowsHide: true,
...
ipcMain.handle('userWorks:start', async (_event, payload) => {
  const { taskId, secUid, nickname, cookies, limit, proxy, config, forceRefresh } = payload;
  console.log(`[userWorks:start] taskId=${taskId} secUid=${secUid ? secUid.slice(0, 30) : 'none'} limit=${limit} forceRefresh=${forceRefresh}`);
  if (!secUid) {
